...
| 年齢確認 | ⚠ MVP（自己申告） |
| バーコード検索 | ✅ 稼働（2026-08-09 に本番復旧。商品側のJAN登録は未着手） |
...
#### 8.2 マイグレーション本番適用 — ✅ 解決済み（2026-08-09）

本番 Supabase（`sowcdgpznbviookdpexx`）の**実オブジェクト**を照合した結果、
不足していたのは `20260417000000_add_barcode.sql` の1件のみだった。

| 確認項目 | 結果 |
|---|---|
| `reserve_lot_stock` RPC | ✅ あり（決済は正常） |
| `products.barcode` | ❌ 無し → **適用して解消** |
| `buying_agents.target_barcode` | ❌ 無し → **適用して解消** |
| `products.age_restricted` / `restriction_type` | ✅ あり |
| `user_profiles.age_verified_at` / `date_of_birth` | ✅ あり |
| `handle_new_user` の admin ハードコード | ✅ 修正済み |
| `user_profiles.role` DEFAULT | ✅ `'pending'` |
| `creator_lp_designs` SELECTポリシー | ✅ 2件 |
| `creator-assets` バケット上限 | ✅ 100MB |

**発覚した実害**: `products.barcode` 不在により
`/api/products/search` が本番で **HTTP 500** を返し続けていた。
同APIは `select` 句で常に `barcode` を参照するため、
バーコード検索だけでなく**キーワード検索も道連れで停止**していた。
`932b9e7`（2026-04）以降、本番で一度も機能していなかった。
2026-08-09 に列を追加して復旧を確認済み。

**副次的な発見**: 既存商品の `barcode` は全て NULL。
バーコード検索を実運用するには Partner 側での JAN/EAN 登録が必要。

#### 8.2b `schema_migrations` の台帳ドリフト（運用ルール）

- **現象**: 本番の `supabase_migrations.schema_migrations` の最終エントリは
  `20260401000000`。しかし実際には以降の7件のうち**6件が適用済み**だった。
  Supabase SQL Editor から手動適用した際に台帳へ記録していないため
- **根拠**: `20260618000000_fix_handle_new_user_role.sql` のヘッダに
  「本番修正を migration として確定する」「別途 SQL Editor から手動実行済み」
  と明記されている
- **実害**:
  1. **台帳が実態を示さないため調査が誤誘導される。**
     2026-08-09 の調査では、台帳の欠落から
     「年齢確認が本番で無効」「権限昇格が残存」と誤って推定した。
     実オブジェクトを見て初めて両方とも適用済みと判明した
  2. `supabase db push` 実行時に**適用済みDDLを再実行する事故**の恐れ
- **対応（2026-08-09 実施）**: 実在を確認した7件を台帳へ登録して同期
- **今後の運用ルール**:
  - 本番へアドホックSQLを流したら、**同じセッションで台帳にも記録する**
    ```sql
    insert into supabase_migrations.schema_migrations (version, name)
    values ('<version>', '<name>') on conflict (version) do nothing;
    ```
  - 台帳を「適用済みか」の判断根拠にしない。
    スキーマ確認は `information_schema` / `pg_proc` / `pg_policies` を見る
  - 恒久対策としては CI での自動適用（`supabase db push`）へ寄せ、
    手動適用そのものを無くすのが望ましい
...
#### 8.8 バーコード検索 RPC への統合 — SQL本体が未記述
- **現状**: `20260417000000_add_barcode.sql` の 8-10行目は
  コメントだけで **SQL本体が書かれていない**:
  ```sql
  -- run_buying_agent RPC にバーコードフィルタを追加
  -- 既存RPCを更新: barcode が指定されている場合、そのバーコードの商品のみマッチ
  -- （本番適用時はSupabase SQL Editorで実行）
  ```
- **問題**: `buying_agents.target_barcode` 列は存在するが、
  `run_buying_agent` がそれを参照しないため**絞り込みが効かない**。
  マイグレーションを適用しても解決しない
- **改善案**: `run_buying_agent` の `CREATE OR REPLACE FUNCTION` を
  新規マイグレーションとして起こす（コメントで済ませない）。
  併せて購買エージェントのテストを追加
- **関連**:
...
1. **8.1 年齢確認の本人確認強化** — 自己申告のみの状態を解消
2. **8.7 iframe 内ログイン導線の実機確認** — 実装は完了。動作検証のみ
3. **8.8 `run_buying_agent` のバーコードフィルタ実装** —
   列だけあってRPCが未対応。SQL本体を書き起こす
...
8. **8.10 E2Eテスト拡充** —
   特に「本番スキーマとコードの乖離」を検知するスモークテスト。
   今回の検索停止は、公開APIを1本叩くだけで4ヶ月前に気付けた

### 完了済み

- ~~8.6 クリエイターLP 年齢確認統合~~ → `5919e80`
- ~~8.2 マイグレーション本番適用確認~~ → 2026-08-09（barcode 1件を適用）
- ~~8.2b 台帳ドリフトの同期~~ → 2026-08-09
...
#### 2026-08-09（2回目 — 本番DB調査）
- 課題8.2 を解決済みに更新。本番の実オブジェクトを照合し、
  不足は `add_barcode` の1件のみと確定。適用して
  `/api/products/search` の 500（検索機能の全停止）を復旧
- 課題8.2b「`schema_migrations` の台帳ドリフト」を新規追加。
  台帳の欠落7件のうち6件は実際には適用済みだった。
  台帳を判断根拠にしないこと、アドホック適用時は台帳にも記録することを
  運用ルールとして明記
- 課題8.8 の実体を訂正。`run_buying_agent` のバーコードフィルタは
  「未組込み」ではなく**マイグレーションにSQL本体が書かれていない**

#### 2026-08-09（1回目 — 実装との照合）
- 実装との照合により記述を修正
