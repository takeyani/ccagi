...
| 24 | 購買エージェント自動実行 | (本書 §3, REQ-BUYING-AGENT-001) | Cronによる定期自動実行、最大20件/バッチ |
| 25 | 販売エージェント | (本書 §31) | 在庫アラート・引合い通知・賞味期限監視の自動モニタリング |
...
| 代理店 | 2% | 代理店の全売上 | `/signup?role=agent&ref=CODE` から登録 |
| クリエイター | 2% | クリエイターの売上 | グループ紹介フィー（別枠） |
...
免許・許可の確認が困難なため、以下のカテゴリは当面取扱禁止とする。

| カテゴリ | 根拠法令 | 必要な免許・許可 | 状態 |
|---------|---------|----------------|------|
| 古物 | 古物営業法 | 古物商許可証 | 禁止 |
| 酒類 | 酒税法 | 酒類販売業免許 | 禁止 |
| たばこ | たばこ事業法 | たばこ小売販売業の許可 | 禁止 |
| 医薬品・医療機器 | 薬機法 | 販売業の許可・届出 | 禁止 |
| 銃砲刀剣類 | 銃刀法 | 所持許可・販売許可 | 禁止 |

免許確認の仕組みが整い次第、段階的に取扱い開始予定。
商品登録画面（`/partner/products/new`, `/partner/products/import`）に禁止注意表示を実装済み。
利用規約（`/legal`）に詳細を記載。
...
パートナー（メーカー/代理店）の販売活動を自動モニタリングし、在庫不足・引合い未対応・賞味期限切れなどのアラートを自動通知する。

### 31.2 パートナー画面

| パス | 概要 |
|------|------|
| `/partner/sales-agent` | 販売エージェントダッシュボード（KPI 4指標 + アラート + ロット一覧） |

KPI: 販売中ロット数 / 未対応引合い / 過去30日販売 / 在庫アラート（5以下）
...
| アラート種別 | 条件 | 通知先 |
|------------|------|--------|
| 在庫アラート | stock <= 5 | パートナーメンバー全員 |
| 引合い未対応リマインダー | 24時間以上未対応（partner_status=新規） | パートナーメンバー全員 |
| 賞味期限アラート | 14日以内に期限到来 | パートナーメンバー全員 |
